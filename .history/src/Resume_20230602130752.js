...
function Resume() {
  ...
  const onSubmit = async (event, result, fileName) => {
    ...
    // const fileElementDown = document.getElementById("fileDown");
    // fileElementDown.classList.add("file-element-Down");
    // fileElementDown.innerText = "다운로드";

    const fileElement = document.querySelector("resume-make");
    fileElement.innerText =
      '<div id="fileElement"><div id="filename"><div id="dateParagraph"></div></div><button id="fileDown"></button></div>';
    fileElement.classList.add("file-element");
