...
function Search() {
  ...
  return (
    <div>
      <div class="SearchBar_CloseIconButtonContainer___lmNa">
        <button type="button">
      ...
      <div class="container" role="presentation">
        <h4 class="SearchResults">추천 검색어</h4>
        <ul class="SearchResults_wrapper">
          {articles.map((article) => (
            <li key={article.id} class="SearchResults_item">
              <button>{article.title}</button>
            </li>
          ))}
        </ul>

        <h4 class="rankingUpdate">
          인기 검색어
          <span class="rankingUpdateTime">16:00 기준</span>
        </h4>

        <ul>
          {search.map((search) => (
            <li key={search.id} class="SearchResults_item">
              <li class="rankingSearch">
                <span class="rankingSearch1">{search.rank}</span>
                <span class="rankingSearch1_job">{search.title}</span>
                <span class="">-</span>
              </li>
            </li>
          ))}
