import React from "react";

function LoginMyInfo() {
  return (
    <div class="MenuPopover_menuWrapper__Sju5Y">
      <div class="MenuPopover_header__u1J9n">
        <svg width="60" height="25" viewBox="0 0 79 32">
          <path
            fill="#24e0a6"
            d="M27.755 5.155A15.953 15.953 0 0015.992 0C7.152 0-.012 7.164-.012 16.004s7.164 16.004 16.004 16.004c4.653 0 8.84-1.985 11.763-5.155A15.953 15.953 0 0031.996 16c0-4.183-1.606-7.992-4.241-10.845z"
          ></path>
          <path
            fill="#438bff"
            d="M51.286 5.155A15.953 15.953 0 0039.523 0c-8.84 0-16.004 7.164-16.004 16.004s7.164 16.004 16.004 16.004c4.653 0 8.84-1.985 11.763-5.155A15.953 15.953 0 0055.527 16c0-4.179-1.61-7.992-4.241-10.845z"
          ></path>
          <path
            fill="#2c5bf2"
            d="M74.812 5.155A15.953 15.953 0 0063.049 0c-8.84 0-16.004 7.164-16.004 16.004s7.164 16.004 16.004 16.004c4.653 0 8.84-1.985 11.763-5.155A15.953 15.953 0 0079.053 16c0-4.183-1.61-7.992-4.241-10.845z"
          ></path>
          <path
            fill="#24e0bc"
            d="M27.755 5.155A15.953 15.953 0 0015.992 0C7.152 0-.012 7.164-.012 16.004s7.164 16.004 16.004 16.004c4.653 0 8.84-1.985 11.763-5.155A15.953 15.953 0 0123.514 16c0-4.183 1.61-7.992 4.241-10.845z"
          ></path>
          <path
            fill="#438bff"
            d="M51.282 5.155C48.359 1.985 44.171 0 39.519 0s-8.84 1.985-11.763 5.155a15.953 15.953 0 014.241 10.853c0 4.187-1.61 8-4.241 10.853 2.923 3.17 7.111 5.155 11.763 5.155s8.84-1.985 11.763-5.155c-2.631-2.853-4.241-6.662-4.241-10.853s1.61-8 4.241-10.853z"
          ></path>
          <path
            fill="#3a68f9"
            d="M27.755 5.155a15.953 15.953 0 00-4.241 10.853c0 4.187 1.61 8 4.241 10.853 2.631-2.853 4.241-6.662 4.241-10.853s-1.606-8-4.241-10.853z"
          ></path>
          <path
            fill="#2c5bf2"
            d="M63.049 0c-4.653 0-8.84 1.985-11.763 5.155a15.953 15.953 0 014.241 10.853c0 4.187-1.61 8-4.241 10.853a15.953 15.953 0 0011.763 5.155c8.84 0 16.004-7.164 16.004-16.004C79.053 7.164 71.885 0 63.049 0z"
          ></path>
          <path
            fill="#0049db"
            d="M51.282 5.155a15.953 15.953 0 00-4.241 10.853c0 4.187 1.61 8 4.241 10.853a15.953 15.953 0 004.241-10.853c.004-4.191-1.606-8-4.241-10.853z"
          ></path>
        </svg>
        <button type="button">
          <span class="SvgIcon_SvgIcon__root__8vwon">
            <svg class="SvgIcon_SvgIcon__root__svg__DKYBi" viewBox="0 0 24 24">
              <path d="M5.93289 4.6068C5.56201 4.33162 5.03569 4.36219 4.69935 4.69853C4.32938 5.0685 4.32938 5.66834 4.69935 6.03831L10.6611 12L4.69935 17.9617L4.60763 18.0679C4.33244 18.4388 4.36302 18.9651 4.69935 19.3015L4.80561 19.3932C5.17649 19.6684 5.7028 19.6378 6.03914 19.3015L12.0009 13.3402L17.9626 19.3015L18.0688 19.3932C18.4397 19.6684 18.966 19.6378 19.3023 19.3015C19.6723 18.9315 19.6723 18.3317 19.3023 17.9617L13.3406 12L19.3023 6.03831L19.3941 5.93206C19.6693 5.56118 19.6387 5.03487 19.3023 4.69853L19.1961 4.6068C18.8252 4.33162 18.2989 4.36219 17.9626 4.69853L12.0009 10.6598L6.03914 4.69853L5.93289 4.6068Z"></path>
            </svg>
          </span>
        </button>
      </div>
      <ul class="MenuPopover_list__BzAsJ MenuPopover_desktop__QcuKl">
        <li class="MenuPopover_hasNewMywantedAlarm__Ij29H">
          <a
            href="/mywanted"
            class=""
            aria-label=""
            data-attribute-id="lnb"
            data-lnb-kind="myWanted"
            data-badge="true"
          >
            <span>MY 원티드</span>
          </a>
        </li>
        <li class="">
          <a
            href="/profile/matching"
            class=""
            aria-label=""
            data-attribute-id="lnb"
            data-lnb-kind="profile"
          >
            <span>프로필</span>
          </a>
        </li>
        <li class="">
          <a
            href="/status/applications"
            class=""
            aria-label=""
            data-attribute-id="lnb"
            data-lnb-kind="applications"
          >
            <span>지원 현황</span>
          </a>
        </li>
        <li class="">
          <a
            href="/profile/status"
            class=""
            aria-label=""
            data-attribute-id="lnb"
            data-lnb-kind="offersReceived"
          >
            <span>제안받기 현황</span>
          </a>
        </li>
        <li class="">
          <a
            href="/profile/likes"
            class=""
            aria-label=""
            data-attribute-id="lnb"
            data-lnb-kind="likes"
          >
            <span>좋아요</span>
          </a>
        </li>
        <li class="">
          <a
            href="/profile/bookmarks"
            class=""
            aria-label=""
            data-attribute-id="lnb"
            data-lnb-kind="bookmarks"
          >
            <span>북마크</span>
          </a>
        </li>
        <li class="">
          <a
            href="/referral"
            class=""
            aria-label=""
            data-attribute-id="lnb"
            data-lnb-kind="refer"
          >
            <span>추천</span>
          </a>
        </li>
        <li class="">
          <a
            href="/profile/point"
            class=""
            aria-label=""
            data-attribute-id="lnb"
            data-lnb-kind="points"
          >
            <span>포인트</span>
          </a>
        </li>
        <li>
          <button
            type="button"
            class="is-logout"
            data-attribute-id="lnb"
            data-lnb-kind="logout"
          >
            <span>로그아웃</span>
          </button>
        </li>
      </ul>
      <ul class="MenuPopover_list__BzAsJ MenuPopover_mobile__nQmPK">
        <li
          data-attribute-id="lnb"
          data-lnb-kind="myWanted"
          data-badge="true"
          class="MenuPopover_hasNewMywantedAlarm__mobile__rkcti"
        >
          <a href="/mywanted" class="" aria-label="">
            <span>MY 원티드</span>
            <div
              class="profilePic"
           
            >
              <span
                class="Badge_Badge__BJTzL"
               
              >
                <svg width="6" height="6" viewBox="0 0 6 6">
                  <g fill="#fff" fill-rule="nonzero">
                    <path
                      d="M6.647 11L6.647 7.259 6.688 7.259 9.158 11 11 11 11 5 9.353 5 9.353 8.357 9.322 8.357 7.089 5 5 5 5 11z"
                      transform="translate(-123 -375) translate(20 365) translate(98 5)"
                    ></path>
                  </g>
                </svg>
              </span>
            </div>
          </a>
        </li>
        <li data-attribute-id="lnb" data-lnb-kind="profile">
          <a href="/profile/matching" class="" aria-label="">
            프로필
          </a>
        </li>
        <li
          class="MenuPopover_divider__8SsNU"
          data-attribute-id="gnb"
          data-gnb-kind="resume"
        >
          <a href="/cv/list" class="" aria-label="">
            이력서
          </a>
        </li>
        <li data-attribute-id="gnb" data-gnb-kind="matchup">
          <a href="/profile" class="" aria-label="">
            매치업
          </a>
        </li>
        <li data-attribute-id="gnb" data-gnb-kind="refer">
          <a href="/referral" class="" aria-label="">
            추천
          </a>
        </li>
        <li data-attribute-id="lnb" data-lnb-kind="applications">
          <a href="/status/applications" class="" aria-label="">
            지원 현황
          </a>
        </li>
        <li data-attribute-id="gnb" data-gnb-kind="freelancer">
          <a
            href="https://www.wanted.co.kr/gigs/experts"
            class=""
            aria-label=""
            target="_blank"
            rel="noopener noreferrer"
          >
            프리랜서
          </a>
        </li>
        <li
          class="MenuPopover_divider__8SsNU"
          data-attribute-id="gnb"
          data-gnb-kind="salary"
        >
          <a href="/salary" class="" aria-label="">
            직군별 연봉
          </a>
        </li>
        <li data-attribute-id="gnb" data-gnb-kind="event">
          <a href="/events" class="" aria-label="">
            이벤트
          </a>
        </li>
        <li data-attribute-id="gnb" data-gnb-kind="community">
          <a href="/community" class="" aria-label="">
            커뮤니티
            <em>
              <svg
                xmlns="https://www.w3.org/2000/svg"
                width="18"
                height="11"
                viewBox="0 0 18 11"
              >
                <g
                  fill="none"
                  fill-rule="evenodd"
                  font-family="AppleSDGothicNeo-SemiBold, Apple SD Gothic Neo"
                  font-size="9"
                  font-weight="500"
                >
                  <g fill="#36F">
                    <g>
                      <g>
                        <g>
                          <g>
                            <g>
                              <g>
                                <text transform="translate(-931.000000, -13.000000) translate(224.000000, 7.000000) translate(210.000000, 6.000000) translate(350.000000, 0.000000) translate(147.000000, 0.000000)">
                                  <tspan x="0" y="8">
                                    New
                                  </tspan>
                                </text>
                              </g>
                            </g>
                          </g>
                        </g>
                      </g>
                    </g>
                  </g>
                </g>
              </svg>
            </em>
          </a>
        </li>
        <li
          class="MenuPopover_divider__8SsNU"
          data-attribute-id="gnb"
          data-gnb-kind="forEmployers"
        >
          <a href="/dashboard">기업 서비스</a>
        </li>
        <li>
          <button
            type="button"
            class="is-logout"
            data-attribute-id="lnb"
            data-lnb-kind="logout"
          >
            <span>로그아웃</span>
          </button>
        </li>
      </ul>
    </div>
  );
}

export default LoginMyInfo;
